...
impl Painted {
    ...
    /// The part of this row a person can actually see.
    ///
    /// egui records a paint command for text that its clip rectangle then throws away, which is
    /// how a scroll area works: the rows past the end are laid out and never reach the screen.
    /// So the rectangle a shape was given is not the rectangle anybody sees, and anything
    /// reasoning about what is on screen has to intersect the two.
    ///
    /// `None` when the clip removed all of it.
    pub fn visible(&self) -> Option<Rect> {
        let seen = self.rect.intersect(self.clip);
        (seen.width() > 0.5 && seen.height() > 0.5).then_some(seen)
    }
}
...
impl Frame {
    ...
    pub fn collisions(&self) -> Vec<(&Painted, &Painted)> {
        // What each row actually shows, not what it was laid out into. A scroll area lays out
        // every row it holds and shows the few that fit, so comparing laid out rectangles
        // reports the rows below the fold as overlapping whatever is drawn down there. That is
        // a collision nobody can see, and a test that fails on one teaches whoever hits it to
        // loosen the test rather than to look.
        let ink: Vec<(&Painted, Rect)> = self
            .text
            .iter()
            .filter(|p| p.is_ink())
            .filter_map(|p| p.visible().map(|seen| (p, seen)))
            .collect();

        let mut out = Vec::new();
        for (i, (a, a_seen)) in ink.iter().enumerate() {
            for (b, b_seen) in ink.iter().skip(i + 1) {
                if a_seen.shrink(1.0).intersects(b_seen.shrink(1.0)) {
                    out.push((*a, *b));
                }
            }
        }
