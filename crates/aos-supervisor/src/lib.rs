...
impl Supervisor {
    ...
    fn wrong_program(
        ...
        //
        // resolve_program rather than a string match, because the allowlist holds resolved
        // paths now. It is the same check a launch makes, so an entry taken off the list, and
        // one whose file has since gone, both read as wrong here too.
        if self.allowed.resolve_program(recorded).is_err() {
            return Some(format!(
