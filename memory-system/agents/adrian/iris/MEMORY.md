...
The handbook for writing GitHub issues. Read this before you do that
work. It is not loaded into your prompt, so read it rather than working from a memory of it.
...
Store durable context only. Never whole emails, secrets, codes, credentials or unnecessary
personal data.
...
|---|---|
| _(nothing yet)_ | This handbook is empty. The index is here so that the first thing written into it gets a row in the same turn. |

An empty handbook is not a problem. A handbook with sections and no rows is, because nobody can
tell what is in it without reading all of it, and then nobody reads it.
